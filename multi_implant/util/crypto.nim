...
# Calculate the XOR of raw bytes with a certain key
# This function handles binary data without UTF-8 conversion issues
proc xorBytes*(data: seq[byte], key: int): string {.noinline.} =
    var k = key
    result = newString(data.len)
    for i in 0 ..< data.len:
        var byteVal = data[i]
        for f in [0, 8, 16, 24]:
            byteVal = byteVal xor uint8((k shr f) and 0xFF)
        result[i] = chr(byteVal)
        k = k +% 1
