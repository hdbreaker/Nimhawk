...
interface NimplantInfo {
  ...
  relay_parent_ip?: string | null;
  relay_parent_ext_ip?: string | null;
  relay_parent_port?: number | null;
  ...
  ({
    ...
    const infoResult = useSWR<NimplantInfo>(
      ...
      swrFetcher,
      {
        ...
        onError: (error) => {
          console.error(
            `SWR Error - Failed to fetch implant #${currentGuid} data:`,
            error,
          );
        },
      },
    ...
    const consoleEndpoint =
      opened && currentGuid
        ? endpoints.nimplantConsole(currentGuid, historyLimit)
        : null;

    const consoleResult = useSWR(consoleEndpoint, swrFetcher, {
    ...
    const updateHistoryLimit = useCallback(
      (newLimit: number, disableRefresh: boolean = false) => {
    ...
    useEffect(() => {
      ...
      console.log(
        `[${currentGuid}] Raw lastCheckin value:`,
        nimplantInfo.lastCheckin,
      );
      ...
      try {
        ...
        if (dateToProcess.match(/^\d{2}\/\d{2}\/\d{4} \d{2}:\d{2}:\d{2}$/)) {
          ...
          if (diffMs < 0) {
            console.log(
              `[${currentGuid}] Date is in the future by ${-diffMs}ms`,
            );
          ...
          if (days > 0) {
            ...
            timeString =
              totalHours === 1 ? "1 hour ago" : `${totalHours} hours ago`;
          ...
          console.log(
            `[${currentGuid}] Date format doesn't match expected pattern`,
          );
    ...
    const handleRefresh = async () => {
      if (!currentGuid) {
        console.log(
          "handleRefresh: No currentGuid available, skipping refresh",
        );
        return;
      }
    ...
    const handleImplantSelect = (implantId: string) => {
      ...
      api
        .get(`${endpoints.nimplants}`)
        .then((response) => {
          // Find the implant with the matching ID
          const targetImplant = response.find(
            (imp: any) => imp.id === implantId || imp.id === Number(implantId),
          );
            ...
            <Stack gap={5} style={{ flex: 1 }}>
              <Group gap="xs" align="center" wrap="nowrap">
                ...
                {nimplantInfo && (
                  <>
                    <StatusIndicator
                      isActive={nimplantInfo?.active}
                      lastCheckin={nimplantInfo?.lastCheckin}
                    />
                    {nimplantInfo?.workspace_name &&
                    nimplantInfo.workspace_name !== "Default" ? (
                      <Badge
                        color="blue"
                        variant="light"
                        size="xs"
      ...
      >
        ...
        >
          {nimplantInfo?.active && nimplantInfo?.disconnected ? (
            ...
            <>
              Are you sure you want to delete this implant from the database?
        ...
        >
          <Tabs.List px="md" mb="xs">
            <Tabs.Tab value="console" leftSection={<FaTerminal size={15} />}>
              Console
            </Tabs.Tab>
            <Tabs.Tab value="info" leftSection={<FaInfoCircle size={15} />}>
              Process
            </Tabs.Tab>
            <Tabs.Tab
              value="network"
              leftSection={<FaNetworkWired size={15} />}
            >
              Network
            </Tabs.Tab>
            <Tabs.Tab value="downloads" leftSection={<FaDownload size={15} />}>
              Downloads
            </Tabs.Tab>
            <Tabs.Tab value="history" leftSection={<FaHistory size={15} />}>
              History
            </Tabs.Tab>
                      ...
                      <Grid.Col span={6}>
                        ...
                        <Text fw={500} style={{ fontFamily: "monospace" }}>
                          {infoResult.isLoading ? (
                            ...
                            nimplantInfo?.ipAddrExt || "Unknown"
                          )}
                        </Text>
                      </Grid.Col>
                      <Grid.Col span={6}>
                        ...
                        <Text fw={500} style={{ fontFamily: "monospace" }}>
                          {infoResult.isLoading ? (
                            ...
                            nimplantInfo?.ipAddrInt || "Unknown"
                          )}
                        </Text>
                      </Grid.Col>
                      <Grid.Col span={6}>
                      ...
                      <Grid.Col span={6}>
                        ...
                        <Text fw={500}>
                          {infoResult.isLoading ? (
                            ...
                            `${nimplantInfo?.sleepTime || "0"} seconds`
                          )}
                        </Text>
                      </Grid.Col>
                      <Grid.Col span={6}>
                        ...
                        <Text fw={500}>
                          {infoResult.isLoading ? (
                            ...
                            `${nimplantInfo?.sleepJitter !== undefined ? nimplantInfo.sleepJitter : "0"}%`
                          )}
                        </Text>
                      </Grid.Col>
                    </Grid>
                        ...
                        >
                          ...
                          <Text fw={700} size="xl" c="blue">
                            {infoResult.isLoading ? (
                              ...
                              formatCompactNumber(stats.commandCount || 0)
                            )}
                          </Text>
                        </Card>
                      </Grid.Col>
                        ...
                        >
                          ...
                          <Text fw={700} size="xl" c="teal">
                            {infoResult.isLoading ? (
                              ...
                              formatCompactNumber(stats.checkinCount || 0)
                            )}
                            {nimplantInfo?.active && (
              ...
              {opened && (
                ...
                >
                  <Box mb="md">
                    ...
                    <Text size="sm" c="dimmed">
                      Files downloaded from this implant
                    </Text>
                  </Box>
...
export default function NimplantDrawer({
  opened,
  onClose,
  guid,
  onKilled,
...
function NimplantOverviewCard({
  np,
  largeScreen,
  onClick,
}: NimplantOverviewCardType) {
  ...
  const needsDetailedInfo =
    np.active && (!np.lastCheckin || np.lastCheckin === "undefined");
  ...
  useEffect(() => {
    ...
    if (
      !np.lastCheckin ||
      np.lastCheckin.includes("1970") ||
      np.lastCheckin.includes("1969")
    ) {
  ...
  return (
    <div
      className={classes.fullRowHover}
      onClick={handleClick}
    ...
    >
      ...
      <Flex px={16} py="md" style={{ width: "100%", alignItems: "center" }}>
        ...
        >
          ...
          >
            {np.active && !np.disconnected && !np.late ? (
              <FaLink size="1.5em" />
            ) : (
              <FaUnlink size="1.5em" />
            )}
        ...
        <Box style={{ width: "23%", paddingLeft: "0" }}>
          <Stack gap={3}>
            <Text>
              {np.username || "Unknown"} @ {np.hostname || "Unknown"}
            </Text>
            <Group
              gap={4}
              align="center"
          ...
          <Stack gap={3}>
            <Group gap="xs" wrap="wrap">
              ...
              {np.relay_role && np.relay_role !== "STANDARD" && (
                <Badge
                  color={
                    np.relay_role === "RELAY_SERVER"
                      ? "green"
                      : np.relay_role === "RELAY_CLIENT"
                        ? "blue"
                        : "gray"
                  }
                  variant="filled"
        ...
        >
          {onClick && (
            <FaAngleRight size="1.5em" className={classes.actionIcon} />
          )}
        </Box>
      </Flex>
    </div>
